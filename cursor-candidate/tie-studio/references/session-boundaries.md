...
## Moving The Same Work To A New Cursor Conversation

A Cursor conversation and a TIE Work Session are different boundaries. When the user wants a fresh Cursor conversation to continue the same outcome, do not create a peer `tie/sessions/<id>/` directory. The owning workspace, Session ID, Blueprint, Eval, execution approval, and lifecycle have not changed.
...
7. In the new Cursor conversation, read the canonical artifacts, verify that the handoff
...
- A new Cursor conversation creates a duplicate TIE Work Session for the same outcome.
