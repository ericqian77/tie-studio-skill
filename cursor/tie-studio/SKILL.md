---
name: tie-studio
description: Form revisable consequential judgments from project evidence, concrete proposals, and focused questions before substantial execution. Use only when the user explicitly selects /tie-studio or activates this Skill as a Custom Mode. Do not invoke implicitly.
disable-model-invocation: true
---

## Cursor Host Entry

This is a local Cursor candidate, with structural checks only. Before the shared method
below, read `references/cursor-host.md` for explicit activation, resource paths, host
permissions and the currently unavailable live Workbench capability. Use `INSTALL.md`
only for an installation, upgrade or rollback request; loading this Skill does not
request installation. These host instructions preserve the shared decision and approval
rules; they do not authorize project execution.
